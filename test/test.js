...
function createDaemon (done) {
  ipfsd.disposable(function (err, node) {
    ...
    node.startDaemon(function (err, ignore) {
      if (err) done(err)
      log('ipfs daemon running: ' + node.apiAddr)

      var ipfs = ipfsApi(node.apiAddr)
      done(null, node, ipfs)
    })
  })
}
...
describe('ipfs node api', function () {
  describe('single instance', function () {
    ...
    before(function (done) {
      ...
      createDaemon(function (err, node, api) {
        if (err) return done(err)
        ipfsNode = node
        ipfs = api
  ...
  describe.only('multiple instances', function () {
    it('id', function (done) {
      this.timeout(120000)

      createDaemon(function (err, node1, api1) {
        if (err) return done(err)

        createDaemon(function (err, node2, api2) {
          if (err) return done(err)

          api1.id(testfile, function (err, id1) {
            if (err) return done(err)
            assert(id1.ID)
            assert(id1.PublicKey)
            log('id1')
            api2.add(testfile, function (err, id2) {
              if (err) return done(err)
              assert(id2.ID)
              assert(id2.PublicKey)
              log('id2')
              assert(id1.ID !== id2.ID)

              node1.stopDaemon(function () {
                node2.stopDaemon(done)
              })
            })
          })
        })
